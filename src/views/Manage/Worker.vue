<template>
  <div class="app-container">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="输入搜索：">
            <el-input v-model="keyword.userName" class="input-width" placeholder="姓名" clearable></el-input>
          </el-form-item>
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button size="mini" class="btn-add" @click="handleAdd()" style="margin-left: 90%" type="success">添加人员</el-button>
    </el-card>
    <div class="mt">
      <el-table :data="tableData" style="width: 100% ; position: relative; z-index: 1; "
        :row-class-name="tableRowClassName" @row-click="rowclick2">
        <el-table-column prop="id" label="人员编号" width="200" />
        <el-table-column prop="userName" label="姓名" width="200" />
        <el-table-column prop="phone" label="手机号" width="200" />
        <el-table-column prop="role" label="岗位" width="200" />
    ...
    <el-dialog v-model="dialogFormVisible" title="修改人员信息">
    <el-form :model="form">
  ...
  <el-dialog v-model="dialogFormInsertWorker" title="增加人员信息">
    <el-form :model="worker">
      <el-form-item label="人员名字">
        <el-input v-model="worker.username" />
      </el-form-item>
      <el-form-item label="人员电话">
        <el-input v-model="worker.phone" />
      </el-form-item>
      <el-form-item label="初始密码">
        <el-input v-model="worker.password" />
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="dialog-footer">
...
const searchBtn = async () => {
  ...
  try {
    let response = await service.get(`/api/Home/GetWorker?UserName=${keyword.userName}&Phone=${keyword.phone}&PageIndex=${searchQuery.value.currentPage}&PageSize=${searchQuery.value.pageSize}`)
    let result = response.data
    if (result && result.list4) {
      tableData.value = result.list4
    }
...
const DelClick = async (row) => {
    ...
    try {
      const token = localStorage.getItem('token');
        const response = await axios.post('https://localhost:44341/api/Home/DelWorker', {
            id: parseInt(row.id)
        },
      {
        headers: {
...
const closeWorker = async (row) => {
    ...
    try {
      const token = localStorage.getItem('token');
        const response = await axios.post('https://localhost:44341/api/Home/InsertWoker',
        worker.value,
      {
        headers: {
          'Content-Type': 'application/json',
