<template>
    <div class="app-container">
        ...
        <div style="margin-top: 15px">
          <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
            <el-form-item label="输入搜索：">
              <el-input v-model="keyword.userName" class="input-width" placeholder="公司名称" clearable></el-input>
            </el-form-item>
      ...
      <el-card class="operate-container" shadow="never">
        ...
        <span>数据列表</span>
        <el-button size="mini" class="btn-add" @click="handleAdd()" style="margin-left: 90%" type="success">添加供应商</el-button>
      </el-card>
      <div class="mt">
        <el-table :data="tableData" style="width: 100% ; position: relative; z-index: 1; "
          :row-class-name="tableRowClassName" @row-click="rowclick2">
          <el-table-column prop="id" label="供应商编号" width="200" />
          <el-table-column prop="supplierName" label="公司名称" width="200" />
          <el-table-column prop="supplierPhone" label="联系方式" width="200" />
          <el-table-column prop="supplierAddress" label="地址" width="200" />
    ...
    <el-dialog v-model="dialogFormInsertWorker" title="增加人员信息">
      <el-form :model="worker">
        <el-form-item label="供应商名称">
          <el-input v-model="worker.supplierName" />
        </el-form-item>
        <el-form-item label="供应商电话">
          <el-input v-model="worker.supplierPhone" />
        </el-form-item>
        <el-form-item label="供应商地址">
          <el-input v-model="worker.supplierAddress" />
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
  ...
  const searchBtn = async () => {
    ...
    try {
      let response = await service.get(`/api/Home/GetSupplier?supplierName=${keyword.userName}&supplierPhone=${keyword.phone}&PageIndex=${searchQuery.value.currentPage}&PageSize=${searchQuery.value.pageSize}`)
      let result = response.data
      if (result && result.list5) {
        tableData.value = result.list5
      }
  ...
  const closeDialogbtn = async (row) => {
      ...
      try {
        const token = localStorage.getItem('token');
          const response = await axios.post('https://localhost:44341/api/Home/UpdateSupplier',
              form.value,
      {
        headers: {
  ...
  const DelClick = async (row) => {
      ...
      try {
        const token = localStorage.getItem('token');
          const response = await axios.post('https://localhost:44341/api/Home/DeleteSupplier', {
              id: parseInt(row.id)
          },
      {
        headers: {
  ...
  const closeWorker = async (row) => {
      ...
      try {
        const token = localStorage.getItem('token');
          const response = await axios.post('https://localhost:44341/api/Home/InsertSupplier',
          worker.value,
      {
        headers: {
          'Content-Type': 'application/json',
