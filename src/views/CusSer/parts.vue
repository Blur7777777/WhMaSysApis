<template>
        ...
        <div style="margin-top: 15px">
            <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
                ...
                <el-form-item label="按时间查找：">
                    <el-date-picker v-model="keyword.date"  value-format='YYYY-MM-DD' type="date" placeholder="请选择时间"  />
                </el-form-item>
    ...
    <div class="mt">
        <el-table :data="tableData" style="width: 100%" :row-class-name="tableRowClassName" @row-click="rowclick2">
            ...
            <el-table-column prop="afterSaleTime" label="售后时间" width="110">
                <template #default="scope">
                    <div style="display: flex; align-items: center">
                        ...
                        <el-popover :width="300"
                            ...
                            <template #reference>
                                {{ scope.row.afterSaleTime.substring(0, 10) }}
                            </template>
                            <template #default>
                                <div class="demo-rich-conent" style="display: flex; gap: 16px; flex-direction: column">
                                    <div>
                                        {{ scope.row.afterSaleTime }}
                                    </div>
                                </div>
                            </template>
                        </el-popover>
            ...
            <el-table-column label="操作" width="200">
                <template #default="{ row }">
                    <el-button type="primary" @click="openDialog(row)" :icon="Edit" circle></el-button>
...
const DelClick = async (row) => {
    try {
        const response = await axios.post('https://localhost:44341/api/Home/DelSale', {
            id: parseInt(row.id)
        });
        console.log(response.data)
        ElMessage({
...
const Search = async () => {
    ...
    try {
        ...
        let response = await service.get(`/api/Home/GetPartAfterSale?id=${encodeURIComponent(keyword.id)}&userName=${encodeURIComponent(keyword.userName)}${orderTimeParam}&itemName=${encodeURIComponent(keyword.itemName)}&customerName=${encodeURIComponent(keyword.customerName)}&PageIndex=${searchQuery.value.currentPage}&PageSize=${searchQuery.value.pageSize}`)
        let result = response.data
        if (result && result.list3) {
            tableData.value = result.list3
        }
...
const closeDialogbtn = async (row) => {
    console.log(form.value)
    try {
        const response = await axios.post('https://localhost:44341/api/Home/UpdateSale',
            form.value
        );
        console.log(form.value)
